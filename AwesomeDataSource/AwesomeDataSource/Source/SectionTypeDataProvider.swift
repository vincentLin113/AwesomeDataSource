...
open class SectionTypeDataProvider<T, R>: CollectionProvider
    where T: SectionModel, R: RowModel
{
    ...
    public func itemForIndexPath(_ indexPath: IndexPath) -> R? {
        return data[safe: indexPath.section]?.row(in: indexPath.item) as? R
    }
    
    public func updateData(in indexPath: IndexPath, value: T) {
